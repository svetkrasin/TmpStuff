...
/*   Updated: 2019/07/31 23:34:46 by ifavi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t	check(t_ctx *ctx, int *prev_line, int *next_line, int lw, ssize_t i)
{
	int		score;
	int		prev;
	...
	while (++i < ctx->size)
	{
		...
		else
		{
			...
			score = 1;
			if (i / lw > 0 && i % lw > 0)
				score += MIN3(prev, prev_line[i % lw], prev_line[i % lw - 1]);
			if (ctx->data[i] == ctx->base.c[OBSTACLE])
				score = 0;
			next_line[i % lw] = score;
			prev = score;
...
int		solve_bsq(const char* filename, int solved)
{
	...
	result = check(&ctx, line[0], line[1], ctx.width + 1, -1);
	if (result)
		print(&ctx, solved);
